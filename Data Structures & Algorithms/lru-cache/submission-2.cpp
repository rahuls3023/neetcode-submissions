class LRUCache {
    ...
    // We use a doubly linked list because the hashmap gives us the address of the node. Using that address, a doubly linked list can delete any node in O(1) time since every node has both prev and next pointers.
    //  A singly linked list cannot do this because it doesn't know the previous node, so it would require traversal, making deletion O(n). 
    //  This is why an LRU Cache uses a hashmap + doubly linked list to achieve O(1) average time for both get() and put().
